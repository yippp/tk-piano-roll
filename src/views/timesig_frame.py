...
class TimeSigFrame(LabelFrame):
    ...
    def _init_ui(self):
        self._beat_count_spinbox = CustomSpinbox(self, from_=1, to=99, width=3)
        self._beat_count_spinbox.set('4')
        self._beat_count_spinbox.on_value_change(self._forward)

        self._beat_unit_var = StringVar()
        values = [str(2 ** i) for i in range(1, 6)]
        self._beat_unit_combobox = Combobox(self, values=values,
        ...
        self._beat_count_spinbox.grid(row=0, column=0, sticky=W)
        self._sep_label.grid(row=0, column=1, sticky=W+N+E+S)
        self._beat_unit_combobox.grid(row=0, column=2, sticky=E)
        self.grid_columnconfigure(1, weight=1)

    def _forward(self, *args):
        beat_count = int(self._beat_count_spinbox.get())
        beat_unit = int(self._beat_unit_combobox.get())
